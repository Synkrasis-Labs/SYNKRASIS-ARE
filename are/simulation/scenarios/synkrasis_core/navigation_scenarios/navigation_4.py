...
class CustomScenario(COREScenario):
    ...
    prompt: str | None = (
        "Move the player in a square pattern (2 steps right, 2 steps down, 2 steps left, 2 steps up)."
    )
    ...
    def build_events_flow(self) -> None:
        ...
        with EventRegisterer.capture_mode():
            ...
            oracle2 = (
                navigation_app.move_down(steps=2)
                .oracle()
                .depends_on(oracle1, delay_seconds=2)
            )
            oracle3 = (
                navigation_app.move_left(steps=2)
                .oracle()
                .depends_on(oracle2, delay_seconds=2)
            )
            oracle4 = (
                navigation_app.move_up(steps=2)
                .oracle()
