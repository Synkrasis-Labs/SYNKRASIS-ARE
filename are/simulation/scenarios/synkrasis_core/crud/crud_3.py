from are.simulation.apps.agent_user_interface import AgentUserInterface
from are.simulation.apps.synkrasis_core.crud import CRUDApp
from are.simulation.scenarios.core_scenario import COREScenario
from are.simulation.types import EventRegisterer


class CustomScenario(COREScenario):
    """
    @TODO: Update this docstring.
    """

    prompt: str | None = (
        "Show all users. Then, update the emails of all users with no email address to 'default@example.com'. Pick one of these users and ensure the changes were applied."
    )

    def init_and_populate_apps(self, *args, **kwargs) -> None:
        """Initialize apps and populate with sample data"""

        agui = AgentUserInterface()
        crud_app = CRUDApp()  # already populated with default data

        # setup functions
        crud_app.add_user(name="John", age=30, email=None)
        crud_app.add_user(name="Jane", age=25, email=None)

        # Store apps for scenario use
        self.apps = [
            agui,
            crud_app,
        ]

    def build_events_flow(self) -> None:
        """Define the sequence of events that will occur during the scenario"""

        agui = self.get_typed_app(AgentUserInterface)
        crud_app = self.get_typed_app(CRUDApp)

        with EventRegisterer.capture_mode():
            # User event: User requests task creation
            event1 = agui.send_message_to_agent(
                content=self.prompt,
            ).depends_on(None, delay_seconds=2)
            oracle1 = crud_app.list_users().oracle().depends_on(event1, delay_seconds=2)
            oracle2 = (
                crud_app.update_user_email(
                    user_id="John_id", email="defaul@example.com"
                )
                .oracle()
                .depends_on(oracle1, delay_seconds=2)
            )
            oracle3 = (
                crud_app.update_user_email(
                    user_id="Jane_id", email="defaul@example.com"
                )
                .oracle()
                .depends_on(oracle2, delay_seconds=2)
            )
            oracle4 = (
                crud_app.verify_user_field(
                    user_id="John_id",
                    field="email",
                    expected_value="defaul@example.com",
                )
                .oracle()
                .depends_on(oracle3, delay_seconds=2)
            )

            # branching for different orders and verification choices
            oracle5 = (
                crud_app.update_user_email(
                    user_id="Jane_id", email="defaul@example.com"
                )
                .oracle()
                .depends_on(oracle1, delay_seconds=2)
            )
            oracle6 = (
                crud_app.update_user_email(
                    user_id="John_id", email="defaul@example.com"
                )
                .oracle()
                .depends_on(oracle5, delay_seconds=2)
            )
            oracle7 = (
                crud_app.verify_user_field(
                    user_id="John_id",
                    field="email",
                    expected_value="defaul@example.com",
                )
                .oracle()
                .depends_on(oracle6, delay_seconds=2)
            )

            oracle8 = (
                crud_app.verify_user_field(
                    user_id="Jane_id",
                    field="email",
                    expected_value="defaul@example.com",
                )
                .oracle()
                .depends_on(oracle3, delay_seconds=2)
            )

            oracle9 = (
                crud_app.verify_user_field(
                    user_id="Jane_id",
                    field="email",
                    expected_value="defaul@example.com",
                )
                .oracle()
                .depends_on(oracle6, delay_seconds=2)
            )

        self.events = [
            event1,
            oracle1,
            oracle2,
            oracle3,
            oracle4,
            oracle5,
            oracle6,
            oracle7,
            oracle8,
            oracle9,
        ]


if __name__ == "__main__":
    from are.simulation.scenarios.utils.cli_utils import run_and_validate

    # Run the scenario in oracle mode and validate the agent actions
    run_and_validate(CustomScenario())
