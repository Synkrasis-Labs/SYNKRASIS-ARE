...
class CustomScenario(COREScenario):
    ...
    prompt: str | None = (
        "Multiply 6 by 7. Then, add 8 to the product. Then, divide the result by 4 raised to the second power."
    )
    ...
    def build_events_flow(self) -> None:
        ...
        with EventRegisterer.capture_mode():
            ...
            oracle1 = (
                computations_app.multiply_numbers(a=6, b=7)
                .oracle()
                .depends_on(event1, delay_seconds=2)
            )
            oracle2 = (
                computations_app.add_numbers(a=42, b=8)
                .oracle()
                .depends_on(oracle1, delay_seconds=2)
            )
            oracle3 = (
                computations_app.power(base=4, exponent=2)
                .oracle()
                .depends_on(oracle2, delay_seconds=2)
            )
            oracle4 = (
                computations_app.divide_numbers(a=50, b=16)
                .oracle()
