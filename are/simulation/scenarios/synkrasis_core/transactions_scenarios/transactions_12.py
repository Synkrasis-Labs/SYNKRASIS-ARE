...
class CustomScenario(COREScenario):
    ...
    prompt: str | None = (
        "Create account 'G888', deposit 500, then charge a fee of the account's entire amount and check the balance. Finally, close the account."
    )
    ...
    def build_events_flow(self) -> None:
        ...
        with EventRegisterer.capture_mode():
            ...
            oracle1 = (
                transactions_app.create_account(account_id="G888")
                .oracle()
                .depends_on(event1, delay_seconds=2)
            )
            oracle2 = (
                transactions_app.deposit(account_id="G888", amount=500)
                .oracle()
                .depends_on(oracle1, delay_seconds=2)
            )
            oracle3 = (
                transactions_app.charge_fee(account_id="G888", amount=500)
                .oracle()
                .depends_on(oracle2, delay_seconds=2)
            )
            oracle4 = (
                transactions_app.check_balance(account_id="G888")
                .oracle()
                .depends_on(oracle3, delay_seconds=2)
            )
            oracle5 = (
                transactions_app.close_account(account_id="G888")
                .oracle()
