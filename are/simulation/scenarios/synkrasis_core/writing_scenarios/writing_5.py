...
class CustomScenario(COREScenario):
    ...
    prompt: str | None = (
        "Construct a sentence where 'the blue cat jumps above a small X'. Replace 'X' with the available synonym word for 'large plant' in your 'nouns' list."
    )
    ...
    def build_events_flow(self) -> None:
        ...
        with EventRegisterer.capture_mode():
            ...
            oracle2 = (
                writing_app.add_adjective(adjective="blue")
                .oracle()
                .depends_on(oracle1, delay_seconds=2)
            )
            oracle3 = (
                writing_app.add_noun(noun="cat")
                .oracle()
                .depends_on(oracle2, delay_seconds=2)
            )
            oracle4 = (
                writing_app.add_verb(verb="jumps")
                .oracle()
                .depends_on(oracle3, delay_seconds=2)
            )
            oracle5 = (
                writing_app.add_preposition(preposition="above")
                .oracle()
