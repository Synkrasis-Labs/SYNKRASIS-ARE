...
class CustomScenario(COREScenario):
    ...
    prompt: str | None = "Check if 'notes.txt' exists, then read its content."
    ...
    def build_events_flow(self) -> None:
        ...
        with EventRegisterer.capture_mode():
            ...
            oracle1 = (
                file_management_app.file_exists(filename="notes.txt")
                .oracle()
                .depends_on(event1, delay_seconds=2)
            )
            oracle2 = (
                file_management_app.read_file(filename="notes.txt")
                .oracle()
