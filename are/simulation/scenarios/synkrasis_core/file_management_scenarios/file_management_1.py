...
class CustomScenario(COREScenario):
    ...
    prompt: str | None = (
        "Create a file named 'new_meeting.txt'. Then write in it: 'Meeting at 10 AM'."
    )
    ...
    def build_events_flow(self) -> None:
        ...
        with EventRegisterer.capture_mode():
            ...
            oracle1 = (
                file_management_app.create_file(filename="new_meeting.txt")
                .oracle()
                .depends_on(event1, delay_seconds=2)
            )
            oracle2 = (
                file_management_app.write_file(
                    filename="new_meeting.txt", content="Meeting at 10 AM"
                )
                .oracle()
