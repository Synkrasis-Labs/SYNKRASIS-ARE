...
class CustomScenario(COREScenario):
    ...
    prompt: str | None = (
        "Move the player right by 3 steps. Reset the player's position and confirm their location."
    )
    ...
    def build_events_flow(self) -> None:
        ...
        with EventRegisterer.capture_mode():
            ...
            oracle1 = (
                navigation_app.move_right(steps=3)
                .oracle()
                .depends_on(event1, delay_seconds=2)
            )
            oracle2 = (
                navigation_app.reset_position()
                .oracle()
                .depends_on(oracle1, delay_seconds=2)
            )
            oracle3 = (
                navigation_app.get_player_position()
                .oracle()
