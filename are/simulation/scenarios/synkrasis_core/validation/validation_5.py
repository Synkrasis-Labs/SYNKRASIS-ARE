...
class CustomScenario(COREScenario):
    ...
    prompt: str | None = (
        "Validate the email 'invalid-email', hash the password 'test123', then generate an OTP of length 9."
    )
    ...
    def build_events_flow(self) -> None:
        ...
        with EventRegisterer.capture_mode():
            ...
            oracle1 = (
                validation_app.validate_email(email="invalid-email")
                .oracle()
                .depends_on(event1, delay_seconds=2)
            )
            oracle2 = (
                validation_app.hash_password(password="test123")
                .oracle()
                .depends_on(oracle1, delay_seconds=2)
            )
            oracle3 = (
                validation_app.generate_otp(length=9)
                .oracle()
