...
class CustomScenario(COREScenario):
    ...
    prompt: str | None = (
        "Create both 'A123' and 'B456' accounts in that order, deposit 500 into 'A123', then transfer 300 from 'A123' to 'B456' and confirm both balances starting from 'A123'."
    )
    ...
    def build_events_flow(self) -> None:
        ...
        with EventRegisterer.capture_mode():
            ...
            oracle1 = (
                transactions_app.create_account(account_id="A123")
                .oracle()
                .depends_on(event1, delay_seconds=2)
            )
            oracle2 = (
                transactions_app.create_account(account_id="B456")
                .oracle()
                .depends_on(oracle1, delay_seconds=2)
            )
            oracle3 = (
                transactions_app.deposit(account_id="A123", amount=500)
                .oracle()
                .depends_on(oracle2, delay_seconds=2)
            )
            oracle4 = (
                transactions_app.transfer(
                    sender_id="A123", receiver_id="B456", amount=300
                )
                .oracle()
                .depends_on(oracle3, delay_seconds=2)
            )
            oracle5 = (
                transactions_app.check_balance(account_id="A123")
                .oracle()
                .depends_on(oracle4, delay_seconds=2)
            )
            oracle6 = (
                transactions_app.check_balance(account_id="B456")
                .oracle()
