...
class CustomScenario(COREScenario):
    ...
    prompt: str | None = (
        "Create account 'D001', deposit 1000, check its balance and then close it."
    )
    ...
    def build_events_flow(self) -> None:
        ...
        with EventRegisterer.capture_mode():
            ...
            oracle2 = (
                transactions_app.deposit(account_id="D001", amount=1000)
                .oracle()
                .depends_on(oracle1, delay_seconds=2)
            )
            oracle3 = (
                transactions_app.check_balance(account_id="D001")
                .oracle()
                .depends_on(oracle2, delay_seconds=2)
            )
            oracle4 = (
                transactions_app.close_account(account_id="D001")
                .oracle()
