...
class CustomScenario(COREScenario):
    ...
    prompt: str | None = (
        "Create account 'Cainhurst', deposit 100, then refund 100 and retrieve the transaction history."
    )
    ...
    def build_events_flow(self) -> None:
        ...
        with EventRegisterer.capture_mode():
            ...
            oracle1 = (
                transactions_app.create_account(account_id="Cainhurst")
                .oracle()
                .depends_on(event1, delay_seconds=2)
            )
            oracle2 = (
                transactions_app.deposit(account_id="Cainhurst", amount=100)
                .oracle()
                .depends_on(oracle1, delay_seconds=2)
            )
            oracle3 = (
                transactions_app.refund(account_id="Cainhurst", amount=100)
                .oracle()
                .depends_on(oracle2, delay_seconds=2)
            )
            oracle4 = (
                transactions_app.get_transaction_history(account_id="Cainhurst")
                .oracle()
