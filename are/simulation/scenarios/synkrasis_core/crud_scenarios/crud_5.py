...
class CustomScenario(COREScenario):
    ...
    prompt: str | None = (
        "Create a new user named 'Eve' with an age of 22 and empty email. Then, update her email to 'eve@example.com'. Finally delete the user and verify the deletion by checking all the existing users."
    )
    ...
    def build_events_flow(self) -> None:
        ...
        with EventRegisterer.capture_mode():
            ...
            oracle1 = (
                crud_app.add_user(name="Eve", age=22, email=None)
                .oracle()
                .depends_on(event1, delay_seconds=2)
            )
            oracle2 = (
                crud_app.update_user_email(user_id="Eve_id", email="eve@example.com")
                .oracle()
                .depends_on(oracle1, delay_seconds=2)
            )
            oracle3 = (
                crud_app.delete_user(user_id="Eve_id")
                .oracle()
                .depends_on(oracle2, delay_seconds=2)
            )
            oracle4 = (
                crud_app.list_users().oracle().depends_on(oracle3, delay_seconds=2)
            )
