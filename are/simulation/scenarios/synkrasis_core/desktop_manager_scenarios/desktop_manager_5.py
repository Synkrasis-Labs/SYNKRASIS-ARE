...
class CustomScenario(COREScenario):
    ...
    prompt: str | None = (
        "Open 'Terminal', run an 'execute_command' action, and then list all currently open applications."
    )
    ...
    def build_events_flow(self) -> None:
        ...
        with EventRegisterer.capture_mode():
            ...
            oracle1 = (
                desktop_manager_app.open_application(app_name="Terminal")
                .oracle()
                .depends_on(event1, delay_seconds=2)
            )
            oracle2 = (
                desktop_manager_app.perform_action(
                    app_name="Terminal", action="execute_command"
                )
                .oracle()
                .depends_on(oracle1, delay_seconds=2)
            )
            oracle3 = (
                desktop_manager_app.print_open_applications()
                .oracle()
