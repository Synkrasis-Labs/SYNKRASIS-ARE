...
class CustomScenario(COREScenario):
    ...
    prompt: str | None = (
        "Open 'Browser', perform a 'Search' action, then print actions performed in the Browser."
    )
    ...
    def build_events_flow(self) -> None:
        ...
        with EventRegisterer.capture_mode():
            ...
            oracle1 = (
                desktop_manager_app.open_application(app_name="Browser")
                .oracle()
                .depends_on(event1, delay_seconds=2)
            )
            oracle2 = (
                desktop_manager_app.perform_action(app_name="Browser", action="Search")
                .oracle()
                .depends_on(oracle1, delay_seconds=2)
            )
            oracle3 = (
                desktop_manager_app.print_application_actions(app_name="Browser")
                .oracle()
