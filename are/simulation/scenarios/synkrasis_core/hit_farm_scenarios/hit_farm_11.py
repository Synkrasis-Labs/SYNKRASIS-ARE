...
class CustomScenario(COREScenario):
    ...
    prompt: str | None = (
        "Precision drill wheat on D1 and D2 at 20 cm rows, 3.8 cm depth, 8 cm spacing.After completing seeding, return to central hub."
    )

    def init_and_populate_apps(self, *args, **kwargs) -> None:
        agui = AgentUserInterface()
        state = HitFarmState()
        rover = GroundRover(env=state, device_id='rover-1')
        hub = CentralHub()
    ...
    def build_events_flow(self) -> None:
        ...
        with EventRegisterer.capture_mode():
            ...
            oracle_3 = rover.set_planter_config(row_spacing_cm=20.0, depth_cm=3.8,
                                                in_row_spacing_cm=8.0).oracle().depends_on(e0, delay_seconds=1)

            captured = [e0, oracle_1, oracle_2, oracle_3]

            o_coordinate_1 = state.get_coordinates(land_name="D1").oracle().depends_on(e0, delay_seconds=0)
            (x, y) = state.lands["D1"].origin
            o_move_1 = rover.move_to(x=x, y=y).oracle().depends_on([oracle_2, oracle_3, o_coordinate_1],
                                                                   delay_seconds=1)
            o_plant_1 = rover.plant_seed(seed_type="wheat").oracle().depends_on(o_move_1, delay_seconds=0)
            captured.extend([o_coordinate_1, o_move_1, o_plant_1])

            o_coordinate_2 = state.get_coordinates(land_name="D2").oracle().depends_on(e0, delay_seconds=0)
            (x, y) = state.lands["D2"].origin
            o_move_2 = rover.move_to(x=x, y=y).oracle().depends_on([oracle_2, oracle_3, o_coordinate_2],
                                                                   delay_seconds=1)
            o_plant_2 = rover.plant_seed(seed_type="wheat").oracle().depends_on(o_move_2, delay_seconds=0)
            captured.extend([o_coordinate_2, o_move_2, o_plant_2])

            # Return to base
            o_return = rover.return_to_base().oracle().depends_on([o_plant_1, o_plant_2], delay_seconds=1)
            captured.append(o_return)
